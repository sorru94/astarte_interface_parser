...
source_files=(
    "astarte_interface_parser/"*.py
)

echo "Analyzing ${#source_files[@]} file(s) with pylint..."
if ! pylint "${source_files[@]}"; then
    error_exit "pylint analysis failed."
fi
...
echo "Analyzing ${#source_files[@]} file(s) with mypy..."
if ! mypy --ignore-missing-imports "${source_files[@]}"; then
    error_exit "mypy analysis failed."
fi
